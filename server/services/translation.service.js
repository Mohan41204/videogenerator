...
// ### 4. NEVER ATTACH TAMIL GRAMMAR TO ENGLISH WORDS — ALWAYS RESTRUCTURE

// An English technical word must always appear as a clean, bare, standalone token. Never attach a Tamil grammatical suffix or case marker to it — not fused, not hyphenated, not even space-separated:

// BAD (all forms banned equally):
// valueaa, value-ஆ, value ஆ, price-ஐ, quantity-ஓட, total-ல, function-க்கு, object-ஐ, amount-ஆ, result-ஆ, application-க்கு, milliseconds-ல
...
// BAD: "இந்த value-ஐ change பண்ணலாம்."
// GOOD: "இந்த value எப்படி change பண்ணுறதுன்னு பாப்போம்."

// b) When you need to refer back to the concept with a case marker, put the marker on a Tamil demonstrative pronoun or noun (இதை, அதை, இதுக்கு, அதுக்கு, மதிப்பு, முடிவு) instead of on the English word. The English word stays bare; the Tamil word next to it carries the grammar:
...
// c) Prefer a predicate construction ("X ஆகும்" — becomes X) over an object-plus-case-marker construction ("store into X"), since it naturally avoids needing a marker on the English word:
...
// d) When a passive or heavily suffixed construction is unavoidable, restructure into two shorter clauses rather than stacking suffixes:
// BAD: "இந்த object-ஐயும்-ஓடவும்..."
// GOOD: Split into two clean sentences, each keeping the English word bare.

// Priority: always prefer the natural restructured sentence. There is no acceptable form where a Tamil suffix touches an English word directly.
...
// Never attach a Tamil suffix directly to the number word or unit (avoid "milliseconds-ல", "GB-க்கு") — restructure the sentence instead.
 ...
 * (and the slides prompt) so all three stay consistent. Previously this text was
 * duplicated in three places and had drifted out of sync — one copy still taught
 * the model to produce the exact fused/hyphenated pattern ("valueஐ", "inputஆ")
 * that this rule is supposed to ban. Fix drift here once, in one place.
 */
...
const validateTTSInput = (text) => {
  ...
  const hyphenatedSuffix = /[A-Za-z0-9]+-[\u0B80-\u0BFF]+/u;
  const adjacentSuffix = /[A-Za-z0-9]+[\u0B80-\u0BFF]+/u;
  // FIX: previously this matched ANY English word followed by a space and any
  // 1-3 character Tamil string, which false-flagged perfectly ordinary
  // sentences like "function ஒரு amount" (ஒரு = "a/one", not a suffix).
  // Now it only matches an explicit list of known bound case-marker tokens.
